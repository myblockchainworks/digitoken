 ...
 * 
 */
package com.aequalis.digi.main;
...
import com.aequalis.digi.blockchainapi.WEBAPI;
import com.aequalis.digi.blockchainapi.WebAPICall;
import com.aequalis.digi.dto.UserDTO;
...
import com.aequalis.digi.service.UserService;
...
public class HomeController {
	...
	public String mywalletScreen(Model model) {
		...
		if(userId != null) {
			...
			user.setTokenBalance(convertToDecimal(WebAPICall.myTokenBalance(user.getBcaddress())));
	...
	public String sendToken(Model model) {
		...
		if(userId != null) {
			User user = userService.findByUserid(Long.parseLong(userId.toString()));
			model.addAttribute("currentUser", user);
			String balance = WebAPICall.myTokenBalance(user.getBcaddress());
			...
			if (toaddress != null && tokenamount != null) {
				if(toaddress.length() == 42 && toaddress.startsWith("0x")) {
					if (!toaddress.equals(user.getBcaddress())) {
						BigDecimal myBalance = new BigDecimal(convertToDecimal(WebAPICall.myTokenBalance(user.getBcaddress())));
						...
						if (myBalance != null && myBalance.compareTo(sendTokenAmount) > 0) {
							...
							String result = WebAPICall.transferToken(user.getBcaddress(), privateKey, toaddress, convertToFullDecimal(tokenamount));
							if (result != null) {
								if (!result.contains("Error:")) {
									TransactionLog transactionLog = new TransactionLog();
									transactionLog.setTransactiondate(new Date());
