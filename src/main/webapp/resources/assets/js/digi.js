...
function buyTokenConfirmation() {
	...
	if (ether != undefined && ether != '') {
		if (ether >= 1) {
			if(confirm ("Are you sure you want to buy Digi for " + ether + " Ether?")) {
				$('#buyTokenForm').submit();
			}
		} else {
			alert ("Minimum contribution is 1 ether!");
		}
		
	} else {
		alert ("Please enter the ether amount to buy Digi!");
	}
}
...
function sendTokenConfirmation() {
	...
	if (toaddress != undefined && toaddress != '' && tokenamount != undefined && tokenamount != '') {
		if (tokenamount == 0) {
			alert ("Please enter a valid Digi amount!");
		} else {
			if(confirm ("Are you sure you want to send " + tokenamount + " Digi to " + toaddress + "?")) {
				$('#sendTokenForm').submit();
			}
		}
		
	} else {
		alert ("Please fill the recipient address and Digi amount!");
	}
}